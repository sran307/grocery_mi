                    ...
                    if($orders) {
                        ...
                        if($orders->save()) {
                            if($orders->cancel_approved == 1) {
                                $text = "Your Order Cancel Request is Accepted. Plz note the Order Code - ".$orders->order_code.", ecambiar.com";
                                $subject = "Cancel Order Request Accepted";
                            } elseif ($orders->cancel_approved == 2) {
                                $text = "Your Order Cancel Request is Rejected. Plz note the Order Code - ".$orders->order_code.", ecambiar.com";
            ...
            if($privil) {
                ...
                if ($validator->fails()) {
                    ...
                    if($user) {
                        ...
                        } else if(($data['return_type'] == "Exchange") || ($data['return_type'] == "Replacement")) {
                            if (isset($data['det_return_type']) && count($data['det_return_type']) != 0) {
                        ...
                        if(sizeof($grv['details']) != 0) {
                            foreach ($grv['details'] as $key => $value) {
                                ...
                                $details.='<tr class="gj_tr_det" id="gj_tr_det_'.($key+1).'">
                                    ...
                                    <td>
                                        <input type="text" name="det_h_totalprice[]" class="det_h_totalprice" value="'.$value->return_qty * $value->return_amount.'" placeholder="Enter Total Price" disabled>

                                        <input type="hidden" name="det_totalprice[]" class="det_totalprice" value="'.$value->return_qty * $value->return_amount.'" placeholder="Enter Total Price">
                                    ...
                                    foreach ($order_detail as $key => $value) {
                                        ...
                                        if($stock && $stock->created_user != 1) {
                                            if($stock->Creatier->user_type == 2 || $stock->Creatier->user_type == 3) {
                                                ...
                                                $comis = AdminCommision::Where('order_code', $order_code)->Where('order_dets', $value->id)->Where('product_id', $value->product_id)->Where('merchant_id', $stock->Creatier->id)->first();
                                                if($comis) {
                                                    $comis->order_code   = $order_code;
                                                    $comis->order_dets   = $value->id;
                                                    $comis->product_id   = $value->product_id;
        					...
        					if($orders->save()) {
                                if($orders->order_status == 3 || $orders->order_status == 5) {
                                    if($orders->order_status == 3) {
                                        $text = "Your Order has been Delivered. Plz note the Order Code - ".$orders->order_code.",Ecambiar.";
                                    } else {
                                        $text = "Your Order has been Cancelled. Plz note the Order Code - ".$orders->order_code.",Ecambiar.";
                                    }
