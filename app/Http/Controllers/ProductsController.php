...
public function resize_image($path, $width=207, $height=207, $update = false) {
        ...
        if($loged) {
            ...
            if($privil) {
            	...
            	$rules = array(
                    ...
                    'sub_sub_cat_name'     => 'nullable',
                    'package_dimension'=>'nullable',
                    // 'manufacturer'         => 'required',
                    'tags'                 => 'nullable',
                    'original_price'       => 'required|numeric',
                    'tax'                  => 'required|numeric',
                    // 'mrp_price_retailer'         => 'required|numeric',
                    // 'mrp_price_dealer'           => 'required|numeric',
                    // 'discount_price_dealer'=>'required|numeric',
                ...
                if ($validator->fails()) {
            		return View::make('products.product.add_product')->withErrors($validator)->with(array('attributes'=>$attributes, 'store'=>$store, 'page'=>$page));
                } else {
                    $data = Input::all();
                    ...
                    if($product) {
        	           ...
        	           // $product->wattage         = $data['product_wattage'];	 
        	            $product->is_related_product       = $data['is_related_product'];	
        	           ...
        	           // $product->product_cost           = $data['product_cost'];	 
                        ...
                        // $product->discount_price_dealer       = $data['discount_price_dealer'];
        	            $product->service_charge         = $data['service_charge'];	 
        	            $product->tax_type               = $data['tax_type'];	 
        	            $product->original_retailer_price         = $data['mrp_price_retailer'];
        	           // $product->original_dealer_price         = $data['mrp_price_dealer'];
        	            $product->onhand_qty             = $data['onhand_qty'];	 
        	            $product->measurement_unit       = $data['unit'];	 
        	           ...
        	           // $product->package_dimension=$data['package_dimension'];
...
$product->store              = 0;	
                        ...
                        if(isset($img_files)) {
                            ...
                            $img_files->move($file_path, $file_name);
                              $this->resize_image($file_path.'/'.$file_name, 207, 207, true);
                        ...
                        if($product->save()) {
        		            ...
        		            $stock->is_block      =  1;
        		            $stock->save();
                            if(($product->is_related_product == 1))
                        {
                            if(count($request->related_pdts)>0)
                            {
                                foreach($request->related_pdts as $value)
                                {
                                    $old=new RelatedProducts();
                                    $old->product_id=$product->id;
                                    $old->related_product_id=$value;
                                    $old->save();
            ...
            if($privil) {
                ...
                if($product) {
                    ...
                    // 'mrp_price_dealer'           => 'required|numeric',
        	            ...
        	            'tax_type'             => 'required',
        	           // 'shiping_charge'       => 'required|numeric',
        	             'package_dimension'           => 'nullable|integer',
        	           'unit'     => 'required',
        	            'features'             => 'required',
        	            'shiping_policy'       => 'required',
        	            'attributes_flag'      => 'required',
...
$product->store              = 0;	
                        ...
                        if($product->save()) {
        					...
        					if(($product->is_related_product == 1))
                        {
                            if($request->related_pdts && count($request->related_pdts)>0)
                            {
	...
	public function manage_pro_widget()
	{
		$loged = session()->get('user');
        if($loged) {
            $privil = DB::table('previlages as A')
            ...
            if($privil) {
                $page = "Products";
                $data = ProductWidget::all();
            	return View::make("products.product.pro_widget")->with(array('data'=>$data, 'page'=>$page));
            } else {
                Session::flash('message', 'You Are Not Access This Module!'); 
                Session::flash('alert-class', 'alert-danger');
                return redirect()->back();
	...
	public function edit_pro_widget($id)
	{
		$loged = session()->get('user');
        if($loged) {
            $privil = DB::table('previlages as A')
            ...
            if($privil) {
                $page = "Products";
                $data = ProductWidget::where("id", $id)->first();
            	return View::make("products.product.edit_pro_widget")->with(array('data'=>$data, 'page'=>$page));
            } else {
                Session::flash('message', 'You Are Not Access This Module!'); 
                Session::flash('alert-class', 'alert-danger');
                return redirect()->back();
	...
	public function update_pro_widget(Request $request)
	{
       ...
       // dd($request->all());
        $result=ProductWidget::where("id", $id)->update([
            "heading"=>$heading,
			"content"=>$content
        ]);
        if($result) {
            ...
            Session::flash('alert-class', 'alert-danger');
            return redirect()->route('manage_pro_widget');
        } else {
