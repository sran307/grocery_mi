        ...
        if($loged) {
            ...
            if($privil) {
            	...
            	if($log) {
        			...
        			} else if($log->user_type == 3 || $log->user_type == 2) {
                        $banks = BankDetails::Where('merchant_id', $log->id)->get();
                        if(sizeof($banks) == 1) {
                            $bank = BankDetails::Where('id', $banks[0]->id)->first();
                            if($bank && ($bank->default != 1)) {
                                $bank->default = 1;
                                $bank->save();
                                $banks = BankDetails::Where('merchant_id', $log->id)->get();
                            }
            ...
            if($privil) {
            	...
            	if($log) {
        	    	...
        	    	if($banks) {
        				...
        				Session::flash('message', 'You Are Not Access to View!'); 
        				Session::flash('alert-class', 'alert-danger');
        	            return redirect()->route('bank_details');
        			}
        		} else {
