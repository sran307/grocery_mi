            	...
            	if($log) {
            		if ($log->user_type == 1) {
        		        ...
        		        if(sizeof($cash) != 0) {
                            ...
                            $outstand_b = Cashout::Where('merchant_id', $log->id)->Where('paid_status', '!=', 'Paid')->sum('amount_paid');
                            
                            if($outstand) {
                                $cash->{'outstand'} = $outstand - $outstand_b;
                            } else {
                                $cash->{'outstand'} = 0.00;
                            }
        				...
        				$last_request_amount = Cashout::Where('merchant_id', $log->id)->pluck('request_amount')->last();
    		        	if($last_request_amount) {
    		        		$cash->{'last_request_amount'} = $last_request_amount;
    		        	} else {
    		        		$cash->{'last_request_amount'} = "-----";
    		        	...
    		        	$last_request_date = Cashout::Where('merchant_id', $log->id)->pluck('request_date')->last();
    		        	if($last_request_date) {
    		        		$cash->{'last_request_date'} = date('d-F-Y', strtotime($last_request_date));
    		        	} else {
    		        		$cash->{'last_request_date'} = "-----";
            ...
            if($privil) {
            	...
            	if($log) {
            		if ($log->user_type == 2 || $log->user_type == 3) {
            			$comis = AdminCommision::Where('merchant_id', $log->id)->Where('paid_status', '=', 0)->OrderBy('id', 'DESC')->get();
                        ...
                        if($outstand) {
                            $comis->{'outstand'} = $outstand - $outstand_b;
                        } else {
                            $comis->{'outstand'} = 0.00;
                        }
                        ...
                        $last_request_amount = Cashout::Where('merchant_id', $log->id)->pluck('request_amount')->last();
        	        	if($last_request_amount) {
        	        		$comis->{'last_request_amount'} = $last_request_amount;
        	        	} else {
        	        		$comis->{'last_request_amount'} = "0.00";
        	        	}
                        ...
                        if(sizeof($comis) != 0) {
                            $admin_amount = AdminCommision::Where('merchant_id', $log->id)->Where('paid_status', '!=', 1)->sum('amount');
                            $vendor_amount = AdminCommision::Where('merchant_id', $log->id)->Where('paid_status', '!=', 1)->sum('merchant_amount');

                            foreach ($comis as $key => $value) {
                                if($value->type == "Order") {
                                    if($value->order_dets) {
                                        $value->{'qty'} = $value->ComisOdrDets->order_qty;
                                        $value->{'totalprice'} = $value->ComisOdrDets->totalprice;
                                    } else {
                                        $value->{'qty'} = "";
                                        $value->{'totalprice'} = "";
                                    }
                                    ...
                                    if($value->cn_id) {
                                        if(isset($value->CNotes->grv_id)) {
                                            $value->{'qty'} = GrvOrdersDetails::Where('grv_id', $value->CNotes->grv_id)->Where('return_type', 'Refund')->Where('grv_issued', 'Yes')->sum('return_qty');
                                            $value->{'totalprice'} = GrvOrdersDetails::Where('grv_id', $value->CNotes->grv_id)->Where('return_type', 'Refund')->Where('grv_issued', 'Yes')->sum('return_amount');
                                        } else {
                                            $value->{'qty'} = "";
                                            $value->{'totalprice'} = "";
                                        }
            	...
            	if($log) {
            		if ($log->user_type == 1) {
        		        ...
        		        if($cash) {
        	        		...
        	        		if(sizeof($ch_req) != 0) {
                                foreach ($ch_req as $key => $value) {
                                    if($value->type == "Order") {
                                        if($value->order_dets) {
                                            $value->{'qty'} = $value->CashOdrDets->order_qty;
                                            $value->{'totalprice'} = $value->CashOdrDets->totalprice;
                                        } else {
                                            $value->{'qty'} = "";
                                            $value->{'totalprice'} = "";
                                        }
    		...
    		if ($log->user_type == 1) {
    			...
    			if($cash) {
			        ...
			        if ($validator->fails()) {
		        		...
		        		if($data['bal_amount_tranfer'] <= $data['amount_paid']) {
        ...
        if($user) {
            if ($user->user_type == 1) {
                if($start_date && $end_date && $p_status && $vendor) {
			        ...
			        if(sizeof($cash) != 0) {
                        $outstand = AdminCommision::Where('merchant_id', $vendor)->Where('paid_status', '!=', 1)->sum('merchant_amount');

                        $outstand_b = Cashout::Where('merchant_id', $vendor)->Where('paid_status', '!=', 'Paid')->sum('amount_paid');
                        ...
                        $last_request_amount = Cashout::Where('merchant_id', $vendor)->pluck('request_amount')->last();
			        	if($last_request_amount) {
			        		$cash->{'last_request_amount'} = Cashout::pluck('request_amount')->last();
			        	} else {
			        		$cash->{'last_request_amount'} = "-----";
			        	}

                        $last_request_date = Cashout::Where('merchant_id', $vendor)->pluck('request_date')->last();
			        	if($last_request_date) {
			        		$cash->{'last_request_date'} = date('d-F-Y', strtotime($last_request_date));
			        	} else {
			        		$cash->{'last_request_date'} = "-----";
			        ...
			        if(sizeof($cash) != 0) {
                        ...
                        $outstand_b = Cashout::Where('merchant_id', $user->id)->Where('paid_status', '!=', 'Paid')->sum('amount_paid');
                ...
                if($start_date && $end_date && $p_status) {
			        ...
			        if(sizeof($cash) != 0) {
                        ...
                        $last_request_amount = Cashout::Where('merchant_id', $user->id)->pluck('request_amount')->last();
			        	if($last_request_amount) {
                        ...
                        $last_request_date = Cashout::Where('merchant_id', $user->id)->pluck('request_date')->last();
			        	if($last_request_date) {
			        		$cash->{'last_request_date'} = date('d-F-Y', strtotime($last_request_date));
			        	} else {
			        		$cash->{'last_request_date'} = "-----";
