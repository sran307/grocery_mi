    ...
    public function OrderByAdminCom () {
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if($user) {
                    if ($user->user_type == 1) {
                        $comis = AdminCommision::groupBy('order_code', 'cn_id')->selectRaw('*, sum(`amount`) as `comis`, sum(`merchant_amount`) as `comis_vendor`')->OrderBy('id', 'DESC')->paginate(10);

                        if(sizeof($comis) != 0) {
                            $admin_com_total = AdminCommision::sum('amount');
                            $vendor_com_total = AdminCommision::sum('merchant_amount');
                        }

                        return View::make("user.orderby_admin_comis")->with(array('comis'=>$comis, 'admin_com_total'=>$admin_com_total, 'vendor_com_total'=>$vendor_com_total, 'page'=>$page));
                    } else if ($user->user_type == 2 || $user->user_type == 3) {
                        $comis = AdminCommision::groupBy('order_code', 'merchant_id', 'cn_id')->selectRaw('*, sum(`amount`) as `comis`, sum(`merchant_amount`) as `comis_vendor`')->Where('merchant_id', $user->id)->OrderBy('id', 'DESC')->paginate(10);
                        
                        if(sizeof($comis) != 0) {
                            $admin_com_total = AdminCommision::Where('merchant_id', $user->id)->sum('amount');
                            $vendor_com_total = AdminCommision::Where('merchant_id', $user->id)->sum('merchant_amount');
                        }
        ...
        if($user) {
            if ($user->user_type == 1) {
                ...
                if($start_date && $end_date && $order_code) {
                    ...
                    $comis = AdminCommision::Where('merchant_id', $user->id)->OrderBy('id', 'DESC')->Where('order_code', 'like', '%' . $order_code . '%')->paginate(1);
