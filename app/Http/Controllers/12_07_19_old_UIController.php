...
class old_12_07_19_UIController extends Controller
{
    protected $respose;
    ...
    public function MyAccount () {
        ...
        if($value) {
            if($value->user_type == 4) {
                ...
                $re_orders = Orders::Where('replace_order', "Yes")->where('user_id',$value->id)->whereNotIn('order_status',[4,5])->paginate(12);
                if(count($re_orders) != 0) {
                    foreach ($re_orders as $keyzz => $valuezz) {
                        $valuezz['details'] = OrderDetails::Where('order_id', $valuezz->id)->get();
        ...
        if($request->ajax() && isset($request->id)) {
            ...
            if($id != 0) {
                ...
                if ($cancel) {
                    ...
                    if($c_date >= $n_date) {
                        $cancel->cancel_approved = 2;
                        $cancel->cancel_date = $n_date;
                        if($cancel->save()) {
                            $text = "Your Order Cancel Request is received. Our Admin Team has approved your request soon!, ecambiar.com";
                            $text = urlencode($text);
                                ...
                                $txt = '<div class="gj_mail" style="width: 500px; padding: 20px 30px; margin: 0 auto; position: relative; background-image: url('.asset('images/shadow.png').'); background-repeat: no-repeat; height: 100%;  background-size: 100% 102%;">
                                        ...
                                        <div style="padding: 5px; color: #333; margin: 0px 20px; text-align: center; font-size: 18px;">
                                            ...
                                            <table align="center" style=" text-align: center;">
                                                ...
                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Cancel Request Date</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$cancel->cancel_date.'</td>
                                                </tr>
                                            </table>

                                            <p>Your Order Cancel Request is received. Our Admin Team has approved your request soon!</p>
                                            <p>Thank You.</p>
                                             <div style="padding:10px 0px 0px; border-bottom: 1px solid #ff5c00;"> </div>
                                            <p>Thanks & Regards,</p>
                                            <p><a href="'.route('home').'">'.$site_name.'</a></p>
            ...
            if($re_odr) {
                ...
                if($re_odr->save()) {
                    ...
                    if (isset($data['det_id']) && sizeof($data['det_id']) != 0) {
                        ...
                        foreach ($data['det_id'] as $key => $value) {
                            if($value) {
                                ...
                                if(isset($data['return_amount'][$key])) {
                                    ...
                                    $re_odr_details->return_amount = NULL;
                                }
                                ...
                                $re_odr_details->order_returned  = "Yes";
                        ...
                        if($ok == 1) {
                            Session::flash('message', 'Your Return or Replacement Order Successfully, Our Admin Team Verify and Contact you soon!'); 
                            Session::flash('alert-class', 'alert-success');
                            return Redirect::to('/my_account/#Section4');
                        } else {
                            $re_odr->delete();
                            ...
                            if($att_name && $att_value) {
                                ...
                                if($att_qty >= $qty) {
                                    ...
                                    if (array_key_exists($id, $cartData)) {
                                        ...
                                        $t_price = round(($qty * ($price + (($price * $product->tax)/100))),2);
                                            ...
                                            if($product) {
                                                ...
                                                if($onhand_qty && $onhand_qty != 0 && $onhand_qty > 0) {
                                                    if($onhand_qty >= $qty) {
                                                        ...
                                                        $error = 1; 
                                                        $cartData = ($session->get('cart')) ? $session->get('cart') : array();
                                                        if (array_key_exists($o_id, $cartData)) {
                                                            foreach ($cartData as $index => $data) {
                                                                if ($data['offer_id'] == $o_id) {
    ...
    public function DeleteCart( Request $request) {
        $id = 0;
        if($request->ajax() && isset($request->id) && isset($request->cart_id)){
            $id = $request->id;
            ...
            if($id) {
                ...
                if($product){
                    ...
                    if (array_key_exists($id, $cartData)) {
                        ...
                        if($users) {
                            if($users->user_type == 4) {
                                $carts = Carts::Where('id', $cart_id)->Where('user_id', $users->id)->Where('product_id', $id)->delete();
                                if($carts) {
    ...
    public function CheckCutOffs( Request $request) { 
        ...
        if($request->ajax() && isset($request->sum) && isset($request->shc) && isset($request->sc) && isset($request->is_cod)) {
            $sum = $request->sum;
            $shc = $request->shc;
            $sc = $request->sc;
            ...
            if(sizeof($cutoff) != 0) {
                ...
                $tot = round($tot, 2);
                $data = array('error' => '1', 'sum' => $sum, 'shc' => $shc,'sc' => $sc,'cod_amount' => $cod_amount,'tot' => $tot); 
            }
        }
                    ...
                    if($order) {
                        ...
                        if(count($cart) != 0) {
                            ...
                            $product_ships = DB::table('carts')
                                ->select(DB::raw('sum(carts.shiping_charge) AS ship_total'))
                                ->join('products', 'products.id', '=', 'carts.product_id')
                                ->where('carts.user_id', $data['user_id'])
                                    ...
                                    if($order_detail) {
                                        foreach ($order_detail as $key => $value) {
                                            ...
                                            $details.= '<tr>
                                                ...
                                                <td style="font-size: 14px;font-weight: bold;color: black;border: 1px solid black;text-align: center;"> '.$value->unitprice.'</td>
                                                <td style="font-size: 14px;font-weight: bold;color: black;border: 1px solid black;text-align: right;"> '.$value->totalprice.'</td>
                                            </tr>';
                                        }
                                    }
                                    ...
                                    $txt = '<div class="gj_mail" style="width: 500px; padding: 20px 30px; margin: 0 auto; position: relative; background-image: url('.asset('images/shadow.png').'); background-repeat: no-repeat; height: 100%;  background-size: 100% 102%;">
                                        ...
                                        <div style="padding: 5px; color: #333; margin: 0px 20px; text-align: center; font-size: 18px;">
                                            ...
                                            <table align="center" style=" text-align: center;">
                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Customer Name</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$customer_name.'</td>
                                                </tr>

                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Contact No</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$contact.'</td>
                                                </tr>

                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Address</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$address.'</td>
                                                </tr>

                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Order Code</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$order_code.'</td>
                                                </tr>

                                                <tr>
                                                    <th style=" text-align: center;text-transform: uppercase;padding-bottom: 12px;color: #333;font-weight:bold;width: 120px;">Order Date</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: #333;padding-bottom: 12px;"> : '.$order_date.'</td>
                                                </tr>
                                            </table>

                                            <table style="width: 570px;border: 1px solid black;">
                                                <tr>
                                                    <th style="width: 100px;text-align: center;text-transform: uppercase;padding-bottom: 5px;color: black;border: 1px solid black;">Product Title</th>
                                                    <th style="width: 100px;text-align: center;text-transform: uppercase;padding-bottom: 5px;color: black;border: 1px solid black;">Quantity</th>
                                                    <th style="width: 100px;text-align: center;text-transform: uppercase;padding-bottom: 5px;color: black;border: 1px solid black;">Price</th>
                                                    <th style="width: 100px;text-align: right;text-transform: uppercase;padding-bottom: 5px;color: black;border: 1px solid black;">Total</th>
                                                </tr>'.$details.'
                                                <tr>
                                                    <th colspan="3" style="width: 100px;text-align: right;text-transform: uppercase;padding-bottom: 5px;color: black;border: 1px solid black;padding-right: 10px;">Net Total</th>
                                                    <td style="font-size: 14px;font-weight: bold;color: black;border: 1px solid black;text-align: right;"> '.$net_tot.'</td>
                                                </tr>
                                            </table>
