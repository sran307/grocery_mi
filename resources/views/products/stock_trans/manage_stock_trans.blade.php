...
@section('title', 'Manage Stock Transaction')
@section('content')
        ...
        <div class="col-md-9 col-sm-9 col-xs-12">
            <!-- <div class="row">
                <div class="col-lg-12">
                    <ul class="breadcrumb">
                        ...
                        <li class="active"><a> Manage Stock Transaction  </a></li>
                    </ul>
                </div>
            </div> -->
            ...
            <div class="gj_box dark">
                ...
                <header>
                    ...
                    <h5 class="gj_heading"> Manage Stock Transaction  </h5>
                </header>
                        ...
                        <table class="table table-bordered table-striped" id="gj_mge_stock_trans_table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Prev Qty</th>
                                    <th>Current Qty</th>
                                    <th>Attribute Prev Qty</th>
                                    <th>Attribute Current Qty</th>
                                    <th>Date</th>
                                    <th>remarks</th>
                                </tr>
                            </thead>
                                ...
                                @if($stock_trans)
                                    ...
                                    @foreach($stock_trans as $key => $value)
                                        <tr>
                                            ...
                                            <td>
                                                @if($value->order_code)
                                                    @if(isset($value->StockOrders['id']))
                                                        <a href="{{ route('view_orders', ['id' => $value->StockOrders['id']]) }}">{{$value->order_code}}</a>
                                                    @else
                                                        {{$value->order_code}}
                                                    @endif
                                                @else
                                            ...
                                            <td>
                                                @if($value->product_id)
                                                    @if(isset($value->StockProducts->product_title))
                                                        <a href="{{ route('view_product', ['id' => $value->product_id]) }}">
                                                            {{$value->StockProducts->product_title}}

                                                            @if($value->att_name) ({{$value->StockAttName->att_name}} @if($value->att_value) : {{$value->StockAttValue->att_value}}) @endif  @endif 
                                                        </a>
                                                    @else
                                                        {{'------'}}
                                                    @endif
                                            ...
                                            <td>{{$value->att_previous_qty}}</td>
                                            <td>{{$value->att_current_qty}}</td>
