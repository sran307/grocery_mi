        ...
        view()->composer('*', function ($view) {
            ...
            if (\DB::connection()->getDatabaseName()) {
                if (Schema::hasTable('genrals') && Schema::hasTable('seos')) {
                    ...
                    $seoset = Seo::first();
                    $title = Genral::findOrFail(1)->project_name;
                    ...
                    $code=Genral::first()->currency_code;
                    ...
                    $pirmarycategories=Category::
                     select('categories.*')->where('categories.status','=','1')->get();
                     $country = Country::all();
                     $homecategories=Category::
                      select('categories.*')->where('categories.status','=','1')->where('is_home','1')->get();
                      $country = Country::all();
                    $view->with(['wallet_system' => $wallet_system, 'seoset' => $seoset,'cms' => $cms, 'defCurrency' => $defCurrency, 'dashsetting' => $dashsetting, 'pincodesystem' => $pincodesystem, 'cur_setting' => $cur_setting, 'config' => $config, 'rightclick' => $rightclick, 'inspect' => $inspect,
                        ...
                        'pirmarycategories'=>$pirmarycategories,'country'=>$country,'homecategories'=>$homecategories,
                        'code'=>$code
                    ]);
